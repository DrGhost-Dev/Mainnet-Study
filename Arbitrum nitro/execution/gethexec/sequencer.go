...
type Sequencer struct {
	...
	// 새로운 트랜잭션들이 가장 먼저 대기하는 채널
	txQueue chan txQueueItem
	// 한 번의 블록 생성 시도에서 처리되지 못한 트랜잭션들이 임시로 보관되는 곳
	txRetryQueue synchronizedTxQueue
	// 이더리움 L1 체인의 헤더 정보를 읽어오는 역할
...
func (s *Sequencer) PublishTransaction(parentCtx context.Context, tx *types.Transaction, options *arbitrum_types.ConditionalOptions) error {
	// 현재 시퀀서 노드가 활성(Active) 상태인지, 아니면 전달자(Forwarder) 상태인지 확인
	_, forwarder := s.GetPauseAndForwarder()
	if forwarder != nil {
		// 이 노드의 sequencer는 활성 상태 아님
	...
	config := s.config()
	// 트랜잭션이 큐(대기열)에서 대기할 수 있는 최대 시간(QueueTimeout)을 가져옴
	...
	// 트랜잭션 처리 결과를 비동기적으로 전달받기 위한 통로(Channel)를 만듬
...
func (s *Sequencer) publishTransactionToQueue(queueCtx context.Context, tx *types.Transaction, options *arbitrum_types.ConditionalOptions, resultChan chan error, isExpressLaneController bool) error {
	...
	// Timeboost(expressLane) : 경매를 통해 우선권을 구매한 트랜잭션에게 확실한 우위를 보장
	if s.config().Timeboost.Enable && s.expressLaneService != nil {
		// ExpressLaneController : 우선권을 가진 트랜잭션
		// expressLaneService.currentRoundHasController() : 이미 다른 '우선' 트랜잭션이 전용 차로(Express Lane)를 차지하고 있는가?
...
func (s *Sequencer) GetPauseAndForwarder() (chan struct{}, *TxForwarder) {
	s.activeMutex.Lock()
	defer s.activeMutex.Unlock()
	// pauseChan : 노드의 일시정시 상태를 외부에서 감시할 수 있는 채널
