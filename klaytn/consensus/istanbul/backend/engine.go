...
// 충분한 수의 위원회 멤버들로부터 최종 합의를 받았는지 확인
...
func (sb *backend) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.BlockScore = defaultBlockScore // default = 1
	...
	// 블록 번호에 해당하는 거버넌스 설정 변수들을 가지고 옴
	pset, err := sb.governance.EffectiveParams(number)
	if err != nil {
		return err
	}
	if number%pset.Epoch() == 0 {
		// 에폭 블록일 때 거버넌스 변경 사항이 있으면 헤더에 저장
	...
	// 현재 내가 제출할 거버넌스 투표(Vote)가 있다면, 그 투표 데이터를 헤더의 Vote 필드에 기록
	...
	// 이더리움이 PoS(Proof-of-Stake)로 전환(The Merge)되면서 도입된 RANDAO 기능을 Klaytn이 수용한 것입니다.
	// 블록 제안자들이 각자 비밀 값을 제출하고, 이 값들을 조합하여 예측 불가능한 랜덤 숫자를 온체인에서 생성하는 메커니즘
	if chain.Config().IsRandaoForkEnabled(header.Number) {
		prevMixHash := headerMixHash(chain, parent)
...
func (sb *backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	...
	// IsRewardSimple : 현재 블록 보상 정책이 simple인지 아닌지 확인
	...
	// 계산이 완료된 보상 내역(rewards 맵)에 따라 각 주소의 계정 잔액을 실제로 증가시켜주는 함수
	...
	// 오직 KIP160 하드포크이거나 KIP103 하드포크일 때 발생
...
func (sb *backend) updateBlock(block *types.Block) (*types.Block, error) {
	...
	// sign the hash
	// 헤더에 서명하기
	// sigHash(header)는 서명해야 할 데이터(헤더의 해시)를 계산합니다
...
func prepareExtra(header *types.Header, vals []common.Address) ([]byte, error) {
	...
	// Vanity 데이터와 합의 데이터를 결합하여 최종 extra-data 반환
